...
=======
  run: async ({ input, ctx }: { input: z.infer<typeof RunAgentRequestSchema>, ctx: any }): Promise<z.infer<typeof RunAgentResponseSchema>> => {
...
=======
        ...
        try {
          ...
          db.insert(toolCallLogsTable).values({
            ...
            error: error ? String(error) : null,
            duration_ms: String(duration),
          }).catch((err: unknown) => console.error("Failed to log tool call", err));
        }
      };

      const result = await agentService.runAgent(input.task, callToolCallback, ctx.user.id, input.policyId);
>>>>>>> fix/detached-head-recovery
