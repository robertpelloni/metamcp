...
import { Check, Download, ExternalLink, Library, Search, Zap } from "lucide-react";
...
export default function RegistryPage() {
  ...
  const { data: registryData, isLoading } = trpc.frontend.registry.list.useQuery({
    query: searchQuery,
  ...
  const handleInstallConfirm = async () => {
    try {
      ...
      if (installItem.template?.env) {
          const missing = Object.entries(installItem.template.env)
            .filter(([key, config]: [string, any]) => config.required && !templateEnv[key]);

          if (missing.length > 0) {
              toast.error(`Missing required environment variables: ${missing.map(m => m[0]).join(", ")}`);
              return;
          }
      }
      ...
      toast.error(`Failed to install server: ${error.message}`);
    }
  };
    ...
    <div className="space-y-6">
          ...
          <SelectContent>
            ...
            {categories?.map((cat) => (
              <SelectItem key={cat} value={cat}>
                {cat.replace(/<[^>]*>?/gm, '')} {/* Strip HTML from category names */}
              </SelectItem>
            ))}
          </SelectContent>
        </Select>
          ...
          {registryData?.items.map((item) => (
            <Card key={item.url} className="flex flex-col">
              ...
              <CardContent className="flex-1 flex flex-col justify-end gap-4">
                <div className="flex flex-wrap gap-1">
                  {item.categories.slice(0, 3).map((cat) => (
                    <Badge key={cat} variant="secondary" className="text-[10px]">
                      {cat.replace(/<[^>]*>?/gm, '').split(' ')[0]} {/* Simplified category display */}
                    </Badge>
                  ))}
                </div>
                <Button
                ...
                >
                  {item.template ? <Zap className="mr-2 h-4 w-4" /> : <Download className="mr-2 h-4 w-4" />}
                  {item.template ? "One-Click Install" : "Install"}
                </Button>
              </CardContent>
            </Card>
          ))}
          {(!registryData?.items || registryData.items.length === 0) && !isLoading && (
      ...
      <Dialog open={!!installItem} onOpenChange={(open) => !open && setInstallItem(null)}>
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            ...
            {installItem?.template ? (
                ...
                <div className="space-y-4 border-t pt-4">
                    <Label className="text-xs font-semibold uppercase text-muted-foreground">Configuration</Label>
                    {Object.entries(installItem.template.env).map(([key, config]: [string, any]) => (
                        <div key={key} className="space-y-2">
                            ...
                            <Input
                                ...
                                onChange={(e) => setTemplateEnv({...templateEnv, [key]: e.target.value})}
                ...
                <>
                    <div className="space-y-2">
                    <Label>Command</Label>
                    <Input value={serverCommand} onChange={(e) => setServerCommand(e.target.value)} placeholder="npx" />
                    </div>
                    <div className="space-y-2">
                    ...
                    <p className="text-[0.8rem] text-muted-foreground">
                        Check the <a href={installItem?.url} target="_blank" className="underline">repository README</a> for installation instructions.
                    </p>
                    </div>
                </>
            )}
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setInstallItem(null)}>Cancel</Button>
