...
export function AutoDiscovery() {
  ...
  const importMutation = trpc.frontend.autoDiscovery.importDiscovered.useMutation();
  ...
  const discoveredSources = scanMutation.data?.result?.sources || [];
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Scan your system for existing MCP configurations (Claude Desktop, Cursor, VS Code).
          </p>
        </div>
      ...
      <div className="grid gap-6">
        {discoveredSources.map((source: any, i: number) => (
            <Card key={i}>
                <CardHeader className="pb-3">
                    <div className="flex justify-between items-start">
                        <div>
                            <CardTitle className="text-base">{source.description}</CardTitle>
                            <CardDescription className="text-xs font-mono mt-1 break-all">
                                {source.path}
                            </CardDescription>
                        </div>
                        {source.status === 'found' ? (
                            <Badge variant="outline" className="bg-green-500/10 text-green-600 border-green-200">Found</Badge>
                        ) : (
                            <Badge variant="outline" className="bg-red-500/10 text-red-600 border-red-200">{source.status}</Badge>
                        )}
                    </div>
                </CardHeader>
                {source.servers && source.servers.length > 0 && (
                    <CardContent>
                        <div className="border rounded-md divide-y">
                            {source.servers.map((server: any) => (
                                <div key={server.name} className="p-3 flex items-center justify-between hover:bg-muted/50 transition-colors">
                                    ...
                                    {!server.alreadyRegistered ? (
                                        <Button size="sm" variant="ghost" onClick={() => handleImport(server.name, source.path)}>
